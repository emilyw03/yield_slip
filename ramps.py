...
def obj_func_full(slope):
    ...
    L1 = Cofactor("L1", [-0.3 + slope*1])
    L2 = Cofactor("L2", [-0.3 + slope*2])
    L3 = Cofactor("L3", [-0.3 + slope*3])
    H1 = Cofactor("H1", [0.3 - slope*1])
    H2 = Cofactor("H2", [0.3 - slope*2])
    H3 = Cofactor("L3", [0.3 - slope*3])
...
def run_single_job(alpha):
    ...
    for t in range(200):
        seed(t * 100 + 500)
        bounds = [
            {'name': 'slopeL', 'type': 'continuous', 'domain': (0, 0.099)}]
        ...
        best_slope = [optimizer.x_opt]
        SSR, events, fluxD, fluxHR, fluxLR = obj_func_full(best_slope)
    ...
    return [alpha, F_t, F_eff_best, F_amnt_best, fluxD_best, fluxHR_best, fluxLR_best, best_slope[0][0]]
...
if __name__ == '__main__':
    ...
    # save data
    columns = ["alpha", "F_t", "F_eff_best", "F_amnt_best", "abs(fluxD)", "fluxHR", "fluxLR", "slope"]
    df = pd.DataFrame(results, columns=columns)
    df.to_csv("slope_fixD_20250320.csv", index=False)
