...
Runs optimization using multiprocessing (parallelized)
'''
...
def obj_func_full(potentials):
    ...
    # Empty arrays for holding results
    TIME=[]
...
def run_single_job(t):
    '''
    scripts for single optimization run - one job to parallelize
    '''
    T = t # dummy parameter that keeps track of run number
    seed(T * 100 + 500)
    bounds = [
    ...
    return [T, F, abs(fluxD), fluxHR, fluxLR]
...
if __name__ == '__main__':
    ...
    with Pool(processes = num_cpus) as pool:
        results = pool.map(run_single_job, range(2))

    # save data
    columns = ["t", "F_t", "abs(fluxD)", "fluxHR", "fluxLR"]
    df = pd.DataFrame(results, columns=columns)
    df.to_csv("DHLOpt_runs_multi_20250213.csv", index=False)
    
    t_end = time.time()
    runtime = t_end - t_start
    print("runtime: ", runtime)

# after running the above, need to minimize and add F_(t) and associated fluxes to df and plot
...
plt.plot(T, F_min, marker='o', linestyle='-', color='blue')
