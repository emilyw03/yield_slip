...
# below is the optimizer that asks the user to specify alpha
# ask user for alpha
user_alpha = float(input("Enter a value between 0 and 1 for alpha: "))

# check if user_alpha is valid before continuing
if 0 <= user_alpha <= 1:
    bounds = [
        {'name': 'L2', 'type': 'continuous', 'domain': (-0.150, -0.050)},
        {'name': 'D1', 'type': 'continuous', 'domain': (-0.450, -0.350)},
        {'name': 'H2', 'type': 'continuous', 'domain': (0.050, 0.150)}]
    ...
    print("="*20)
    print("Best potentials (L2, D1, H2): ", optimizer.x_opt)
    ...
    print("Alpha: ", user_alpha)
    print("Iterations: ", maxiter)
    print("Best square Euclidean dist: ", SSR)
    print("Raw FluxDR: ", fluxD)
    print("Raw FluxLR: ", fluxLR)
    print("Raw FluxHR: ", fluxHR)

    print("="*20)

    # plot of how optimizer explores parameter space
    optimizer.plot_acquisition()
    plt.show()

    # plot of how optimizer evolves over iterations
    optimizer.plot_convergence()
    plt.show()

else:
    print("Invalid alpha. Please enter a value between 0 and 1.")
