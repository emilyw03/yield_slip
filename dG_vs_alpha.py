'''
dG_vs_alpha (likely same result as 3cof_float3 but good check)
Author: Emily Wang
March 2025
Optimizing dG vs. alpha. L3 and H3 aren't fixed
Float D and dG = -(L3 + H3). Midpoint potential on D kept at 0 eV. 
ET branches have 3 cofactors each.
...
def run_single_job(alpha):
    ...
    for t in range(200):
        ...
        bounds = [
            ...
            {'name': 'dG', 'type': 'continuous', 'domain': (-0.400, 0)}]
    ...
    pL3_best = best_params[0][0]
    dG_best = best_params[0][2]
    pH3_best = -pL3_best - dG_best

    return [alpha, F_t, F_eff_best, F_amnt_best, fluxD_best, fluxHR_best, fluxLR_best, 
            best_params[0][1], best_params[0][0], pH3_best, best_params[0][2]]
...
if __name__ == '__main__':
    ...
    # save data
    columns = ["alpha", "F_t", "F_eff_best", "F_amnt_best", "abs(fluxD)", "fluxHR", "fluxLR", "potential_D1", "potential_L3", "potential_H3", "dG"]
    df = pd.DataFrame(results, columns=columns)
    df.to_csv("dG_vs_alpha_20250328.csv", index=False)
