'''
bump_alpha1_corner_update
Author: Emily Wang
June 12, 2025
Only allow dip (i.e. lower limit of pH1 = 300 meV). Optimize bump size (H1) for a grid of effective slope values. Uses Bayesian Optimization.
...
def run_single_job(slopes):
    ...
    for t in range(300):
        seed(t * 100 + 500)
        bounds = [{'name': 'H1', 'type': 'continuous', 'domain': (0.300, 0.500)}]
...
if __name__ == '__main__':
    ...
    # save data
    columns = ["slopeL", "slopeH", "F", "F_slip", "F_yield", "fluxD", "fluxHR", "fluxLR", "potential_H1"]
    df = pd.DataFrame(results, columns=columns)
    df.to_csv(f"BestBump_alpha1_corner_update_{task_id}_"+timestr+".csv", index=False)
