...
if __name__ == '__main__':
    ...
    for t in range(300):
        F_sc = one_run(slopes, t)
        F_sc_data.append(F_sc)
        t_data.append(t)
        F_sc_best_data.append(min(F_sc_data))

    columns = ["t", "F_sc_best", "F_sc"]
    rows = list(zip(t_data, F_sc_best_data, F_sc_data))
    df = pd.DataFrame(rows, columns=columns)
    df.to_csv(f"bump_optFsc_converg_"+timestr+".csv", index=False)
