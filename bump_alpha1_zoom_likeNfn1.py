...
def obj_func_full(potentials, slopes):
    ...
    # Infinite reservoirs - reservoir positions and rates based on Nfn1
...
if __name__ == '__main__':
    ...
    slopeL_vals = np.linspace(0.181, 0.200, grid_size)
    slopeH_vals = np.linspace(-0.169, -0.149, grid_size)
    ...
    # save data
    columns = ["slopeL", "slopeH", "F", "F_slip", "F_yield", "fluxD", "fluxHR", "fluxLR", "potential_H1", "D_H1_flux", "L1_D_flux"]
    df = pd.DataFrame(results, columns=columns)
    df.to_csv(f"bump_alpha1_likeNfn1_zoom_{task_id}_"+timestr+".csv", index=False)
