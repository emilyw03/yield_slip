...
def run_single_job(alpha):
    ...
    for t in range(1):
        ...
        params_evaluated = optimizer.X

    return params_evaluated

if __name__ == '__main__':
    params_eval = run_single_job(0.5)
    df = pd.DataFrame(params_eval, columns = ['slopeL', 'slopeH'])
    df.to_csv('ramps2_iters_20250419.csv', index = False)
    ...
    plt.scatter(slopeL, slopeH, c = 'blue', alpha = 0.7)
    plt.xlabel('slopeL')
    plt.ylabel('slopeH')
    plt.title('(slopeL, slopeH) pairs searched in one Bayesian Optimization run')
    plt.show()