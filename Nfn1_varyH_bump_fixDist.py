...
Date: June 13, 2025
Author: Emily Wang
Nfn-1 modeled with infinite reservoir MEK. Hold all distances = 10 Å
'''
...
def Nfn1(mu_FeS_H1, slopeH, t): 
    ...
    net.addConnection(L_FAD, FeS_L1, dist)   #B-L1
    net.addConnection(FeS_L1, FeS_L2, dist)   #L1-L2
    net.addConnection(L_FAD, FeS_L2, dist*2)     #B-L2
    # High-potential branch
    net.addConnection(L_FAD, FeS_H1, dist)    #B-H1
    net.addConnection(FeS_H1, S_FAD, dist)     #H1-H2
    net.addConnection(L_FAD, S_FAD, dist*2)     #B-H2
    # Other connections
    net.addConnection(FeS_L1, FeS_H1, dist*2)    #L1-H1
    net.addConnection(FeS_L1, S_FAD, dist*3)    #L1-H2
    net.addConnection(FeS_L2, FeS_H1, dist*3)    #L2-H1
    net.addConnection(FeS_L2, S_FAD, dist*4)    #L2-H2
...
if __name__ == '__main__':
    ...
    time = 10**4
    for val in chunk:
        NADPH, NAD, Fd, F_slip, F_yield, F_sc, D_to_H1_flux, L1_to_D_flux = Nfn1(0.080, val, time)
    ...
    df.to_csv(f"Nfn1_varyH_bump_fixDist_{task_id}_20250711.csv", index=False)